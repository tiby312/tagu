...
pub fn renderer()->Renderer<NoFmt>{
    Renderer::new()
}
...
impl<D:Fmt> Renderer<D>{
    ...
    pub fn render<E:Elem+Locked,W:fmt::Write>(&mut self,elem:E,mut writer:W)->fmt::Result{
        ElemWrite(WriteWrap(&mut writer),&mut self.fmt).render(elem)
    }
    pub fn render_escapable<E: Elem, W: fmt::Write>(&mut self,elem: E, mut writer: W) -> fmt::Result {
